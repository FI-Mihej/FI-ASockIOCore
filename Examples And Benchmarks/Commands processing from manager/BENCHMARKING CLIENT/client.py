import marshal
from help_tools import PLATFORM_NAME
from simple_network.tcp_app_server import *
from performance_test_lib.performance_test_lib import test_run_time
...
def run_non_blocking_server_benchmarking_client():
    ...
    with asock_io_core_connect(tcp_app_server) as server:
        ...
        super_server_tcp_settings = ConnectionSettings(ConnectionDirectionRole.server, SERVER_ADDRESS,
                                                       MANAGER_CLIENT_KEYWORD)
        #   If AF_UNIX is allowed and this is Unix-like os
        if BSC__USE_UNIX_SOCKET_ON_CLIENT:
            if 'posix' == os.name:
                super_server_tcp_settings = ConnectionSettings(ConnectionDirectionRole.server, SERVER_ADDRESS_AF_UNIX,
